CREATE TABLE IF NOT EXISTS `dws.dwb_etc_cardtype_week_i_d`( 
week_id                 string comment '自然周期',
...
ALTER TABLE dws.dwb_etc_cardtype_week_i_d DROP IF EXISTS PARTITION(dt='${datekey}'); 
insert overwrite table dws.dwb_etc_cardtype_week_i_d partition(dt= '${datekey}')
select week_id,
		card_type,
...
from (
	select date_format(open.open_time,'yyyy-WW') as week_id,
			card.card_type,
	...
	and date_format(open.open_time,'yyyy-WW')=date_format('${datekey}','yyyy-WW')
	group by date_format(open.open_time,'yyyy-WW'),
			card.card_type
	...
	select date_format('${datekey}','yyyy-WW') as week_id,
			card.card_type,
			0 as open_cards,
	...
	select date_format(x.active_info.active_time,'yyyy-WW') as week_id,
			card.card_type,
			0 as open_cards,
			0 as ac_open_cards,
	...
	where date_format(x.active_info.active_time,'yyyy-WW')=date_format('${datekey}','yyyy-WW')
	group by date_format(x.active_info.active_time,'yyyy-WW'),
			card.card_type
			
	union all
	
	
	select 	date_format(active.perf_active_time,'yyyy-WW') as week_id,
			card.card_type,
			0 as open_cards,
			0 as ac_open_cards,
			0 as active_cards,
	...
	and date_format(active.perf_active_time,'yyyy-WW')= date_format('${datekey}','yyyy-WW')
	group by date_format(active.perf_active_time,'yyyy-WW'),
			card.card_type
	...
	select date_format(cancel.cancel_time,'yyyy-WW') as week_id,
			card.card_type,
			0 as open_cards,
			0 as ac_open_cards,
			0 as active_cards,
	...
	and date_format(cancel.cancel_time,'yyyy-WW') = date_format('${datekey}','yyyy-WW')     
	group by date_format(cancel.cancel_time,'yyyy-WW') ,
			card.card_type
	...
	select date_format(deposit.deposit_time,'yyyy-WW') as week_id,
			card.card_type,
			0 as open_cards,
			0 as ac_open_cards,
			0 as active_cards,
	...
	and date_format(deposit.deposit_time,'yyyy-WW') = date_format('${datekey}','yyyy-WW') 
	group by date_format(deposit.deposit_time,'yyyy-WW'),
			card.card_type
)x
group by week_id,
		card_type;